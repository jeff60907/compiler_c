...
int eval()
{
    ...
    while(1) {
        ...
        if (op == IMM) {
            ax = *pc++;                   // load global address or immedicate
        } else if (op == LC) {
            ax = *(char *)ax;             // load char
        } else if (op == LI) {
            ax = *(int *)ax;              // load int
        } else if (op == SC) {
            ax = *(char *)*sp++ = ax;     // save char
        } else if (op == SI) {
            *(int *)*sp++ = ax;           // save int
        }
            ...
            *--sp = ax;                   // push the value of ax onto the stack
            ...
            pc = (int *)*pc;              // jump to next address
        } else if (op == JZ) {
            pc = ax ? pc+1 : (int *)*pc;  // jump if zero
        } else if (op == JNZ) {
            pc = ax ? (int*)*pc : pc+1;   // jump if not zero
        }
            ...
            *--sp = (int)bp;              // enter subroutine
            ...
            sp = sp + *pc++;              // stack adjust , add esp,<size>
        } else if (op == LEV) {
            sp = bp;                      // leave subroutine and restore
            bp = (int *)*sp++;
            pc = (int *)*sp++;
        } else if (op == LEA) {
            ax = (int)(bp + *pc++);       // load local address for argmuents
        }
